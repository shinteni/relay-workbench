...
- **私有决策检查点与决策库**：封存后仍默认只留内存；只有再次明确点击 `保存私有检查点` 才会把完整的冻结来源、精确 payload 和裁决结果原子写入 `Application Support/Relay/decisions`。目录权限为 `0700`、文件为 `0600`；冷启动后即使没有终端窗口，也可从侧栏决策库恢复只读链路；决策详情顶部的**证据时间线**（检查点 → 行动 → 恢复 → 见证，含人工标注计数）一眼俯瞰整条链。损坏或不兼容文件会被隔离计数，不阻断其他记录；删除前明确确认并移入 macOS 废纸篓，不直接永久删除
...
- **会话记录（Claude/Codex 式）**：侧栏 `会话` 区常驻——按项目分可折叠文件夹（符号链接归一、按最近活动排序、记忆展开状态、自动展开当前项目），文件夹下按标题列出 daemon 自动记录的全部会话，点标题即开：任务组重挂载为对比/串联窗，单任务开成**线程窗**（输出实时跟新 + 底部续聊）；🔍 进会话库面板可搜索（多关键词，忽略大小写/全半角/重音）、行内改名（存回 daemon）、类型过滤与确认删除；嵌入式终端 PTY 内容一如既往不记录
