...
- **项目历史（Claude/Codex 式）**：侧栏按项目显示一套可折叠历史记录（符号链接归一、按最近活动排序、记忆展开状态、自动展开当前项目）；拖动侧栏中部的分界线可调整项目历史占比，位置会自动记住。`＋` 选择文件夹即可添加或恢复项目，项目行 `…` 只从侧栏隐藏、不删除磁盘文件；daemon 自动保存的会话与用户明确保存的私有决策检查点共用标题列表，右键会话可在确认后删除单条记录。点击会话会重开任务组或线程，点击检查点会打开原有只读决策详情；🔍 打开统一历史面板，可搜索会话和决策，或对会话改名、按类型过滤及确认删除。嵌入式终端 PTY 内容一如既往不记录
