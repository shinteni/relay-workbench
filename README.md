...
- **项目坞**：主界面始终显示当前项目，整块点击即可切换最近 6 个本地目录或选择新文件夹；`双开` 一键打开或复用该项目的 Claude + Codex，并在两者独占工作区时自动平铺；之后新开的终端、对话、同时发送与顺序串联统一使用该项目，已打开窗口保持原 cwd
...
- **项目为主体的侧栏（v0.107 重设计）**：项目区是侧栏主体——头部下方即当前项目卡片（切换菜单 + `双开`），其下的按项目历史独占全部弹性高度（符号链接归一、按最近活动排序、记忆展开状态、自动展开当前项目、同名项目自动附父目录消歧、悬停查看完整路径）。`＋` 选择文件夹即可添加或恢复项目，项目行 `…` 可设为当前项目或仅从侧栏隐藏（不删除磁盘文件）；daemon 自动保存的会话与用户明确保存的私有决策检查点共用标题列表，右键会话可在确认后删除单条记录。点击会话会重开任务组或线程，点击检查点会打开原有只读决策详情；🔍 打开统一历史面板，可搜索会话和决策，或对会话改名、按类型过滤及确认删除。嵌入式终端 PTY 内容一如既往不记录
- **底部智能体坞**：智能体从整行列表压缩为底部紧凑芯片格（字标 + 名称 + 健康点；`▣` 徽标示意开着的终端数、彩点示意运行/等待任务）——点击芯片打开该 CLI 终端，右键可直接发起对话，悬停查看版本与健康详情；对话/对比/接力三个串联入口同排收为单行，窗口列表限高滚动，项目区因此始终占侧栏大部分空间
