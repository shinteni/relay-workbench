...
| | |
|---|---|
| **真实终端** | 嵌入的是各 CLI 的**原生 TUI**，不是模拟器：拖动、八向缩放、贴边吸附、一键平铺与双开；输出雷达、待看队列、跨 CLI 行动路由 |
| **五种串联** | ⇄ 会议（圆桌）· ⋈ 并行（同时发送）· › 分工合作（顺序接力）· ⚡ 结果裁决 · ⑂ 上下文分叉 |
| **决策链** | 冻结现场 → 裁决（人握回车）→ 封存 → 私有检查点（含 git 基线）→ 血缘导航与漂移检查，全程显式操作、只读证据 |
| **本地优先** | 无账号、无遥测、无云同步；画面快照仅存内存，检查点以 `0700/0600` 落盘，嵌入式终端 PTY 内容从不记录 |
| **开放接入** | 按行输出的 CLI 走 `generic` 段、讲 ACP 的走 `acp` 段——零代码 manifest 即可接入任意 CLI |
...
当前可直接调用 Codex CLI、Claude CLI、Grok CLI（经 generic manifest 零代码接入，含会话续聊）、本地 Ollama，以及让 Claude 与 Codex 达成最终共识的 MIX 智能体；并可通过通用 Adapter 协议继续接入其他 CLI——按行输出的 CLI 走 generic 段，讲 ACP（Agent Client Protocol）的 CLI 走 acp 段，都是零代码 manifest 接入。仓库保留 [`xai-org/grok-build`](https://github.com/xai-org/grok-build) 快照（grok CLI 的上游项目，Apache-2.0），作为 CLI runtime、ACP 和终端交互实现参考——产品代码不依赖它。
...
- 点击左侧智能体，把它的**原生 CLI TUI**（Claude Code / Codex / Ollama / 任意 generic CLI）作为独立窗口嵌入：像普通应用窗口一样拖动、八向缩放、叠放置顶、贴顶、双击或 `⤢` 最大化/还原；推到画布边缘自动吸附半屏、推到角落吸附四分屏，`−` 可最小化到底部停靠条；最多 4 个终端
- **项目坞**：主界面始终显示当前项目，整块点击即可切换最近 6 个本地目录或选择新文件夹；`双开`（v0.109 起为智能体坞第 6 格芯片）一键打开或复用该项目的 Claude + Codex，并在两者独占工作区时自动平铺；之后新开的终端、对话、同时发送与顺序串联统一使用该项目，已打开窗口保持原 cwd
...
- **本地上下文分叉**：从任一终端标题栏明确截取当时的活动画面，在底部分叉轨道中编辑快照、填写继续指令并选择一个或多个其他原生 CLI；截取含最近约 600 行滚回缓冲，保留 UTF-8 安全的末尾 48 KiB，只存在于当前 GUI 内存，不进剪贴板或磁盘。Relay 仍只用 bracketed paste 填入、不按 Return，随后按实际填入顺序进入逐目标核对并可一键返回来源
...
- **⚡ 一键裁决**：裁决层也可直接选一个 daemon 智能体当裁判——同一份裁决载荷作为后台任务发出，结构化结论返回后直接生成决策并可封存；全程无需开终端、无需手动 Return（"人握回车"的终端裁决路径完整保留）
...
- **私有决策检查点与决策库**：封存后仍默认只留内存；只有再次明确点击 `保存私有检查点` 才会把完整的冻结来源、精确 payload 和裁决结果原子写入 `Application Support/Relay/decisions`。目录权限为 `0700`、文件为 `0600`；冷启动后即使没有终端窗口，也可从侧栏对应项目的历史记录恢复只读链路；决策详情顶部的**证据时间线**（检查点 → 行动 → 恢复 → 见证，含人工标注计数）一眼俯瞰整条链。损坏或不兼容文件会被隔离计数，不阻断其他记录；删除前明确确认并移入 macOS 废纸篓，不直接永久删除
...
- **恢复见证对照**：同一恢复变化下保存至少两份见证后，用户可显式为左右两侧各选一份记录。双端证据秤同屏显示两个真实 CLI 目标、人工标注、接力 / 画面字节，并只作确定性事实判定：准确 handoff payload 是否逐字节相同、人工标注是否分歧。两份见证画面用现有有界逐行差异并排，每侧最多 64 KiB / 300 行且明确提示截尾；对照只存在当前内存，不写新文件、不修改见证，Relay 不选“更好”的结果；对照是证据链的只读终点（v0.78 起收口，不再叠加接力层）
...
- **圆桌对话**（`⇄ 对话` 窗口）：按发言顺序选 2–4 个智能体与主题，Relay 引擎按座次轮转驱动多轮交谈——各自保持独立会话记忆（无会话 CLI 自动降级为带上下文重问），每人开口前收到"此后各方发言"的署名摘要；完成后可 `+1 轮`、以**主持人**身份插话引导新一轮、`导出` 成 Markdown 纪要，或把各方最终立场一键送进裁决/分叉给终端
- **同时发送 / 顺序串联**：同一提示词并行交给 2–4 个智能体对照，或让 2–4 个步骤在 daemon 中按顺序接力（关闭 GUI 后串联继续；重开后侧栏 `↺` 可把 daemon 里的任务组重新挂载为窗口）；串联完成后底部输入框可**续轮对话**（每步保持自身会话），两种窗口的结构化结果都可一键送进裁决或分叉给终端
...
- **项目为主体的侧栏（v0.107–v0.109 重设计）**：头部是一条串联电路——`>` 提示符与当前项目节点由一条信号色走线连成（daemon 状态仅在非在线时才以警示节点现身于走线上），当前项目卡片（环形接线端子 + 项目名 + 路径 + 整卡切换菜单）就是走线的终点；其下的按项目历史独占全部弹性高度（符号链接归一、按最近活动排序、记忆展开状态、自动展开当前项目、同名项目自动附父目录消歧、悬停查看完整路径）。`＋` 选择文件夹即可添加或恢复项目，项目行 `…` 可设为当前项目或仅从侧栏隐藏（不删除磁盘文件）；daemon 自动保存的会话与用户明确保存的私有决策检查点共用标题列表，右键会话可在确认后删除单条记录。点击会话会重开任务组或线程，点击检查点会打开原有只读决策详情；🔍 打开统一历史面板，可搜索会话和决策，或对会话改名、按类型过滤及确认删除。嵌入式终端 PTY 内容一如既往不记录
- **底部智能体坞**：智能体从整行列表压缩为底部紧凑芯片格（字标 + 名称 + 健康点；`▣` 徽标示意开着的终端数、彩点示意运行/等待任务）——点击芯片打开该 CLI 终端，右键可直接发起对话，悬停查看版本与健康详情；`双开` 是网格第 6 格组合芯片（一键为当前项目打开 Claude + Codex 并自动平铺）；对话/对比/接力三个串联入口同排收为单行，窗口列表限高滚动，项目区因此始终占侧栏大部分空间
...
- 私有 Unix socket、进程组取消（对 pgid 被系统回收的 EPERM 场景免疫，不误判任务失败）、连接/任务/输出上限和 Adapter 路径校验
...
1. 在侧栏顶部的项目卡片核对当前项目；点击卡片切换最近项目或选择新文件夹，或点底部智能体坞的 `双开` 芯片一次打开当前项目的 Claude + Codex。项目切换只影响之后新开的窗口。
2. 点击齿轮或按 `⌘,` 打开设置；在 `通用` 中选择界面语言、默认工作目录，并可管理**任务生命周期钩子**（任务完成/失败/等待时执行本地命令，任务信息只经 `RELAY_TASK_*` 环境变量传入）；在 `智能体` 中设置模型与推理强度，并可管理**座次预设**（智能体+选项覆盖+追加规则的命名人格档案）。
3. 点击左侧智能体行，右侧会打开它的原生 CLI；拖动标题栏移动，拖动边缘缩放，双击标题栏最大化/还原。
4. Relay 会随窗口打开、移动、缩放与平铺自动更新 CLI 桌面记忆。重新启动后，在空工作区核对缩略图，再选择 `恢复桌面` 或 `忘记`。
5. 使用左侧 `串联` 区的 `对话 / 对比 / 接力` 打开对应编排窗口；窗口内选择智能体（圆桌与对比也可选**座次预设**），输入主题或任务后开始；勾选 `自检后交付` 会在提示词尾附加可见的自验证指令。对比窗可开启 `⎇ 隔离工作区`——每个成员在独立 git worktree 里实现、实时显示改动数，完成后可全量`采纳此版`或`选择采纳…`按文件/改动块合并；串联完成后可续轮，也可 `⑂ 分叉` 把整条链开成新窗（支持会话分叉的步骤续接会话副本，原窗不动）。
...
9. 有运行中的原生终端时，点击“窗口”旁的投递图标打开提示词暂存台；先逐窗确认 CLI 已停在输入框，再手动勾选目标并点击 `填入 N`。Relay 会清除暂存正文并自动聚焦第一个目标；目标芯片会以铅笔或 Return 标记反馈填入后的本地输入动作，但 Relay 不读取或保存输入内容，也不会代你按 Return。检查终端输入行后点击 `已检查 · 下一窗`；中途可收起 HUD，侧栏投递入口会显示剩余数，点击后从同一目标继续。全部核对后可点 `汇流当前画面`，把至少两个仍打开的目标画面冻结在同一横向面板中；之后可逐卡查看实时 CLI、明确 `重新截取`，或 `返回核对`。要让另一个 CLI 综合这些结果，点 `裁决这些结果`，填写裁决指令后二选一：`⚡ 一键裁决` 选 daemon 智能体当裁判，结构化结论（结论/理由/把握徽章，解析失败回退原文）直接进决策面板；或选择一个安全粘贴已就绪的终端目标，点 `填入裁决目标 · 不执行`；所有冻结结果会一次性填入并进入新的单目标核对。确认裁决 CLI 的可见结果已经完成后，可点 `封存结果` 查看“冻结来源 → 精确载荷 → 裁决结果”决策链；返回核对后可用 `查看封存结果` 原样重开。若要跨重启保留，在决策面板明确点击 `保存私有检查点`（git 项目会同时记录 HEAD 基线与脏标记，详情里可显式`检查漂移`做只读比较）；之后可在侧栏对应项目的历史记录打开只读检查点、为检查点添加独立的标题/标签并置顶、用搜索栏按标题/标签/智能体/项目/证据/结果/ID 过滤、从检查点重新裁决，或经确认移入废纸篓。要把已确认结果带回日常实现，点 `从此决策继续`，核对精确简报字节、选择一个输入就绪 CLI，再点 `填入目标 · 不执行`；完成核对前可用 `返回决策` 原路回到检查点，已填内容不会被撤回。派生检查点会显示重裁决血缘，并在父记录仍存在时提供 `对比结果` 与 `查看父检查点`；存在多代或分支时，可直接点击详情顶部血缘中的祖先或子节点原位切换，也可用 `比较血缘节点` 选择同一家族的其他分支作为参考。每一代仍须独立显式保存。`结束核对` 只清除核对元数据，终端中已经填入的正文不会被改动。
