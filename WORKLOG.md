...
- **仓库**：`github.com/shinteni-hikaso/relay-workbench`（public）——只推 `main` 单线历史；发布前检查：gh 登录（shinteni-hikaso）、dist//.claude 未跟踪、上游 grok-build 快照为 Apache-2.0 可合规再分发、密钥模式扫描无命中。
...
- 分支已整理：Codex 侧栏分支（v0.104–v0.106.2）连同 v0.107.0 已快进合并回 `main` 并删除分支，v0.107–v0.109 与发布准备均已提交，历史保持单线。**已发布**：`origin` = github.com/shinteni-hikaso/relay-workbench（public，仅推 main；本地 `refs/remotes/upstream/main` 的 grok-build 历史不推送），LP = https://shinteni-hikaso.github.io/relay-workbench/（GitHub Pages，main:/docs）。历史里程碑：`ed6e9a4`（→v0.78）、`afcaf83`、`78eb3f7`（→v0.93）、`dc2d62d`（→v0.101.2）、`94ffc24`（文档）、`c362a06`（→v0.103）、`0543822`（交接）、`8b1dcb7`（→v0.104）。
