...
### v0.107.0：侧栏重设计——项目为主体、智能体收进底坞（用户："项目应该是主要内容，占大部分区域，选择模型的部分可以缩小"）

- **布局翻转，结构性取代 v0.106 的手动分界线**：项目区（当前项目卡片 + 项目历史）紧随侧栏头部，独占全部弹性高度；智能体与串联入口固定压缩为底部坞。占比不再需要拖动调节，分界线组件、`sidebarUpperHeight` 偏好与配套辅助功能动作一并移除。
- **当前项目卡片**：项目标题行（`＋` 添加、🔍 检索）下方置显眼卡片——文件夹图标、项目名、路径缩写、切换菜单与 `双开`；"新窗口使用此项目"说明并入悬停提示，不再占一行。
- **智能体芯片格**：五个高行压缩为自适应网格芯片（字标块 + 名称 + 健康点，约两行）；点击开终端语义不变，右键菜单保留"打开终端 / 与它对话"，悬停提示补全版本、健康与任务计数；开着的终端数以 `▣` 徽标、运行/等待任务以彩点显示。串联三按钮（对话/对比/接力）收为单行。
- **窗口区归位**：v0.104–0.106 中"窗口"标题行与窗口列表被项目历史隔开的三明治结构消除，两者现连续排列且列表限高滚动；审批横幅上移至侧栏头部下方常显位置。
- **同名项目消歧**：多个可见项目同名时（实例：`/Users/个人项目/串联器` 与 `/Users/shinn/串联器` 旧史），文件夹行自动附父目录淡色提示，悬停显示完整路径；空项目展开显示"还没有项目历史"；`…` 菜单新增"设为当前项目"。
- 验证：Swift **150** 项（20 套件）全过零警告；release 打包签名通过，覆盖安装冷启动 `0.107.0 (140)`；真机操作验证——点击历史会话重挂载链窗口、点击 Claude 芯片打开真实 TUI（芯片同步选中描边 + `▣` 徽标）、窗口列表限高、关窗回收、`…` 菜单与同名消歧后缀均正常。
- 版本：Relay.app `0.106.2 (139)` → **`0.107.0 (140)`**。分支整理：本条提交落在 Codex 侧栏分支（v0.104–v0.106.2）之上，随后 `main` 快进合并至此并删除该分支，历史恢复单线。

### v0.108.0：头部电路重设计（用户点名"这部分使用 skill 再重新设计一下"）

- **头部即串联电路**：侧栏头部按应用本名重构为一条串联走线——`>` 提示符 → daemon 状态点 → 当前项目节点，三个锚点共用 16pt 锚柱、以 1.5px 信号色细线连成一体；结构自身讲清工作方式（输入从提示符出发、经 daemon、落进当前项目）。字级反转：`RELAY_` 从 20pt 降至 16pt 归位为品牌铭牌，项目名升至 15pt 成为侧栏最大的内容文字。
- **当前项目卡片**：左端为走线终点的环形接线端子（沿用 v0.4x 项目坞的图形语言），项目名 + 完整路径缩写 + 切换菜单 + `双开`；顶缘渐变描边模拟受光；切换项目时端子做一次 0.22s 临界阻尼脉冲（跟随系统减弱动效设置静默）。"新窗口使用此项目"说明并入悬停提示。
- **控件归位**：`＋ 添加 / 🔍 检索` 从卡片上方移到"项目"列表标题行——它们作用于列表而非当前项目，就近映射。
- **修复陈年 Menu 压平缺陷**：macOS `.menuStyle(.borderlessButton)` 会把自定义 label 压平为"图标+标题"，卡片里的节点图形、路径行与布局自 v0.4x 起从未真正渲染（`⌃⌄` 箭头被提升成左侧图标、卡片撑不满宽度）。改为 `.menuStyle(.button)` + `.buttonStyle(.plain)` 由 SwiftUI 渲染完整 label；透明 label 覆盖层方案实测点击无效（NSPopUpButton 命中区随空标题塌陷），已否弃。
- 验证：Swift **150** 项（20 套件）全过零警告；release 打包签名通过，覆盖安装冷启动 `0.108.0 (141)`；真机操作验证——点卡片弹出最近项目菜单、切至连接器再切回（卡片/列表高亮/父目录消歧同步刷新）、`双开`按钮与走线对位正常。
- 版本：Relay.app `0.107.0 (140)` → **`0.108.0 (141)`**。

### v0.109.0：头部再减、双开入坞（用户："daemon 在线可以去掉了，双开按钮放到模型区域"）

- **daemon 状态行按需出现**：正常在线时头部不再显示 `DAEMON 在线` 行——菜单栏常驻入口已覆盖该信息；仅当 daemon **非在线**（连接中/离线）时，警示节点才出现在走线上（StatusDot 琥珀色 + 状态文案 + 版本），沿用"审批只在到期时出现"的哲学。状态切换带临界阻尼过渡，减弱动效时静默。
- **双开移入智能体坞**：项目卡片只留"节点 + 项目名 + 路径 + 切换"，整卡即菜单；`双开` 改为智能体网格的第 6 格芯片（`⧉` 图标块 + 信号色描边，与 AgentChip 同构的 PairChip），5 个智能体 + 双开正好补满 2×3 网格，组合动作紧挨它要打开的 Claude/Codex 芯片。
- 验证：Swift **150** 项（20 套件）全过零警告；release 打包签名通过，覆盖安装冷启动 `0.109.0 (142)`；真机操作——头部无 daemon 行、点 `双开` 芯片 Claude+Codex 同开并自动平铺（两芯片同步选中态 + `▣`）、关窗回收正常。
- 版本：Relay.app `0.108.0 (141)` → **`0.109.0 (142)`**。

### 开源发布：GitHub 仓库、README 主页化与 LP 落地页（用户："上传到我个人的github里，并制作相应的主页介绍和lp网页"）

- **仓库**：`github.com/shinteni/relay-workbench`（public，个人账号）——只推 `main` 单线历史；发布前检查：dist//.claude 未跟踪、上游 grok-build 快照为 Apache-2.0 可合规再分发、密钥模式扫描无命中。**纠错**：首次误推到 gh 当时的活跃账号 shinteni-hikaso（公司身份），用户指出后已将该仓库转私有、撤下其 Pages（原 LP 已 404），全部迁移至个人账号 shinteni 重新发布；公司号下的私有仓库副本待用户自行删除（需 delete_repo 权限）。
- **README 主页化**：顶部新增 hero（定位一句话、LP 链接、亮点表、两条命令快速上手、定位与构成段），原「已实现」全量目录保持不动。
- **LP 落地页**：`docs/index.html` 单文件自包含（零外部依赖、系统字体栈），部署为 GitHub Pages。设计沿用应用本体语言：RelayPalette 换算色板、页面级"串联走线"签名（滚动时节点点亮，减弱动效时静态）、CSS 精绘应用 mockup 作主视觉（侧栏项目卡片＋芯片格＋双 TUI 平铺）；六个区块=真实终端 / 五种串联 / 决策链 / LOCAL ONLY / 零代码接入 / 两条命令上手；移动端走线隐藏、区块全宽。
- **截图与隐私**：制作素材时两次原生 screencapture 误捕到真实桌面（含 Slack 工作内容），均立即删除；窗口级捕获因 CGWindowList 权限受限不可用，最终 LP 采用 CSS mockup 方案，不含任何真实桌面像素。
- 验证：LP 在预览面板桌面/移动两种宽度下检查通过（hero、卡片、决策链、代码块、页脚）。

### v0.110.0：英文界面与三语门面（用户："给应用添加英文界面，以及 lp 和 github 添加日语，英文支持"）

- **应用英文界面**：本地化层的 key 本就是成品英文文案，英文即恒等表——`RelayLanguage` 增 `english`（"en"），`text()` 英文直接返回 key；六个 switch 格式化函数（任务状态/daemon/健康/过滤器/输出角色/Codex 模式）与三个占位符函数补英文分支；设置窗语言选择器统一用 `nativeName` 渲染（简体中文 / 日本語 / English），即时切换。默认语言保持简体中文。
- **LP 三语化**：`docs/index.html` 单文件内置中/英/日——顶栏语言切换器、`data-i18n` 字典（中文留在标记里作基准，切回时从快照还原）、串联演示的提示词/输出/裁决三语脚本（语言切换时演示即时以新语言呈现完成态）；首次访问按 `navigator.language` 自动选择，选择存 localStorage；`html lang` 同步更新。日文术语与应用内表一致（対話/比較/チェーン/判定/承認/円卓/ペア）。
- **README 英日版**：新增 `README.en.md` / `README.ja.md`（hero + 亮点表 + 快速上手 + 定位，全量功能目录指回中文版）；三份 README 顶部互挂语言切换链接。
- **并行会话改动收录**：圆桌对话提示词策略（允许用工具含网络检索、本地内容只读）作为独立提交先行入库。
- 验证：Swift **151** 项（20 套件，新增英文恒等表与格式化函数测试）全过零警告；打包部署 `0.110.0 (143)`，实机切换 English 全界面即时生效（侧栏 PROJECTS/WINDOWS/AGENTS/PAIR/Dialogue/COMPARE/CHAIN/LOCAL ONLY 与设置窗全页）后切回中文；LP 在预览面板逐语言验证 hero/窗口标题/演示脚本。
- 版本：Relay.app `0.109.0 (142)` → **`0.110.0 (143)`**。

### v0.111.0：三大串联入口更名——会议 / 并行 / 分工合作（用户点名）

- **叫法**：对话→**会议**（Meeting / 会議）、对比→**并行**（PARALLEL / 並列）、接力→**分工合作**（TEAMWORK / 分業），三语同步。
- **改动面**：本地化 key 随英文名更名（Dialogue→Meeting、COMPARE→PARALLEL、CHAIN→TEAMWORK），中日表值更新；使用点全量替换——侧栏底坞三按钮、编排/对话窗标题与上下文分叉来源名、会话库过滤片、提示词暂存台窗口类型标签、智能体芯片右键菜单与按钮帮助文案（"开一场智能体会议"/"打开一个双智能体会议窗口"）。机制性描述词（顺序接力、同时发送、圆桌、链）保留不动。
- **LP 与 README**：LP 三语窗口标题同步（分工合作/TEAMWORK/分業、会议/MEETING/会議、并行/PARALLEL/並列）；三份 README 的"五种串联"行改为新名并括注旧称助记。
- 验证：Swift **151** 项全过零警告；打包部署 `0.111.0 (144)`，实机三语轮验底坞按钮（会议/并行/分工合作 → 会議/並列/分業 → Meeting/PARALLEL/TEAMWORK），保持用户当前所选英文界面退出。
- 版本：Relay.app `0.110.0 (143)` → **`0.111.0 (144)`**。
...
### 当前状态（随迭代更新，2026-07-23 更新至 v0.111.0）

- 分支已整理：Codex 侧栏分支（v0.104–v0.106.2）连同 v0.107.0 已快进合并回 `main` 并删除分支，v0.107–v0.109 与发布准备均已提交，历史保持单线。**已发布**：`origin` = github.com/shinteni/relay-workbench（个人账号，public，仅推 main；本地 `refs/remotes/upstream/main` 的 grok-build 历史不推送），LP = https://shinteni.github.io/relay-workbench/（GitHub Pages，main:/docs）；首次误推的公司号仓库已转私有并撤下 Pages。本机 gh 登有两个账号（shinteni-hikaso=公司、日常活跃；shinteni=个人），推送身份跟随 gh 活跃账号——本仓库已在 `.git/config` 配置仓库级凭据助手固定以 shinteni 推送，公司号保持活跃不受影响。历史里程碑：`ed6e9a4`（→v0.78）、`afcaf83`、`78eb3f7`（→v0.93）、`dc2d62d`（→v0.101.2）、`94ffc24`（文档）、`c362a06`（→v0.103）、`0543822`（交接）、`8b1dcb7`（→v0.104）。
- 版本空洞说明：`0.34.0 (55)` 由并行会话（Codex）交付、未记入本 WORKLOG；其改动已包含在工作树与安装包中。
- 当前运行版本：Relay.app **v0.111.0 (144)**（`~/Applications`）、cli-adapters 0.4.0、relay-protocol 0.3.0、relayd **0.9.5**、协议 v8；daemon 在线，注册 Claude / Codex / MIX / Ollama / Grok（grok 经用户目录 generic manifest 接入）；ACP CLI 可经 `acp` manifest 接入（本机装有 claude-code-acp 可作真实 ACP 目标）。
- 产品形态时间线：任务流线程工作台（→ v0.32）→ 嵌入式真 CLI 终端（v0.33）→ 自由浮动窗口（v0.36–v0.37）→ 智能体对话（v0.38）→ 对比/串联/审批窗回归（v0.39–v0.40）→ 侧栏交互重设计（v0.41）→ CLI 桌面记忆（v0.42）→ 项目坞与新窗口 cwd 单一真值（v0.43）→ 终端项目上下文护栏（v0.44）→ 当前项目 Claude + Codex 双开（v0.45）→ 跨 CLI 输出雷达（v0.46）→ 待查看输出队列（v0.47）→ 提示词暂存台（v0.48）→ 逐窗核对回路（v0.49）→ 隐私化编辑/Return 信号（v0.50）→ 可恢复核对胶囊（v0.51）→ 跨 CLI 行动路由器（v0.52）→ 可逆行动跳转（v0.53）→ 焦点电路（v0.54）→ 本地上下文接力（v0.55）→ 上下文分叉（v0.56）→ 结果汇流（v0.57）→ 结果裁决（v0.58）→ 裁决预算回流（v0.58.1）→ 裁决载荷预检（v0.59）→ 裁决血缘回看（v0.60）→ 来源漂移检查（v0.61）→ 裁决结果封存（v0.62）→ 私有决策检查点与决策库（v0.63）→ 检查点重裁决与派生血缘（v0.64）→ 父子决策差异审阅（v0.65）→ 多代决策血缘导航（v0.66）→ 跨分支决策比较（v0.67）→ 私有决策库本地搜索（v0.68）→ 不可变证据旁的私有标签与置顶（v0.69）→ 决策到行动桥（v0.70）→ 决策行动回执（v0.71）→ 行动恢复桥（v0.72）→ 恢复变化回执（v0.73）→ 恢复变化接力（v0.74）→ 恢复变化见证（v0.75）→ 恢复见证对照（v0.76）→ 见证对照接力（v0.77）→ 证据链收口（v0.78）→ daemon 结果接入汇流裁决（v0.79）→ 编排窗重挂载（v0.80）→ 一键裁决/主持人席/纪要导出（v0.86–v0.88）→ 侧栏可收起（v0.89）→ worktree 并行实现与实时改动看板（v0.90/v0.92）→ 审批规则引擎（v0.91）→ 提示词队列（v0.93）→ ACP 通用适配器（v0.94）→ Hunk 级选择性采纳（v0.95）→ 结构化裁决输出（v0.96）→ 自检开关（v0.97）→ 座次预设（v0.98）→ 串联会话分叉（v0.99）→ 检查点代码基线（v0.100）→ 任务生命周期钩子（v0.101）→ 会话库（v0.102）→ 侧栏会话列表与线程窗口（v0.103）→ Codex 式统一项目历史（v0.104）→ 项目侧栏管理与单条历史删除（v0.105）→ 可调项目历史占比（v0.106）→ 项目为主体的侧栏重设计（v0.107，撤销手动分界线，智能体收为底坞芯片格）→ 头部串联电路（v0.108，提示符→daemon→项目走线，修复 Menu label 压平）→ 头部再减与双开入坞（v0.109）→ 英文界面与三语门面（v0.110）→ 串联入口更名会议/并行/分工合作（v0.111，三语同步）。当前工作区六种窗口：终端 / 对话 / 同时发送 / 顺序串联 / 线程 / 审批。
- 测试基线：Rust **113**（protocol 13 + relayd 39 + relayctl 10 + codex 3 + claude 3 + mix 1 + generic 20 + acp 24）/ Swift **151（20 套件）** / MIX 包装 3 / vendored MIX 64。
